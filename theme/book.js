...
(function sidebar() {
  ...
  function toggleSection(ev) {
    ...
    const isExpanded =
      ev.currentTarget.parentElement.classList.contains("expanded");
    try {
      localStorage.setItem(
        `mdbook-sidebar-toggle-${toggleId}`,
        isExpanded ? 1 : 0
      );
    } catch (e) {}
  }
  ...
  Array.from(sidebarAnchorToggles).forEach(function (el, id) {
    ...
    const isActive =
      el.parentElement.nextSibling.querySelector(".active") !== null ||
      el.previousSibling.classList.contains("active");
    if (!isActive) {
      try {
        const isExpanded =
          localStorage.getItem(`mdbook-sidebar-toggle-${id}`) === "1";
        if (!isExpanded) {
